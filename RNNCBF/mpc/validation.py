...
from viz.animation import make_system_obstacle_montage_v1, make_system_obstacle_animation_v2, make_system_obstacle_animation_v3
from npz_builder import NPZBuilder
from viz.animation import plot_traj_segments
from viz.animation import make_system_obstacle_svg_frames_v3
...
def flat_input_fn(mpc, X, horizon, xpred_hor, ypred_hor, m):
    ...
    for t in range(horizon):
        x_t    = X[:,t]
        cbf_t  = [h_func for h_func in mpc.rnn.obst.h_obsfunc(x_t, xpred_hor[t*m:(t+1)*m], ypred_hor[t*m:(t+1)*m])]  # mx1 each
...
def MPC_func(x, mpc, params, solver_inst, xpred_list, ypred_list, hidden_in, m, x_prev, lam_x_prev, lam_g_prev, layers_list):
        ...
        flat_input = flat_input_fn(mpc, X, mpc.horizon, xpred_list, ypred_list, m)
        # mpc.horizon*
...
# )
    ...
    # keep lengths consistent with GIFs
    T_pred = plans.shape[0]
