...
class RLagent:
        ...
        def rnn_warmstart(self, params, warmup_steps=50):
            """
            Roll the closed-loop v_mpc a few steps to produce a good initial hidden state.
            Resets env + obstacle motion.
            """
            self.x_prev_VMPC        = cs.DM()  
            self.lam_x_prev_VMPC    = cs.DM()  
            self.lam_g_prev_VMPC    = cs.DM()  
        ...
        def v_mpc_rand(self, params, x, rand, xpred_list, ypred_list, hidden_in):
            ...
            lbx = np.concatenate([np.array(x).flatten(), 
                                  self.X_lower_bound, 
                                  U_lower_bound,  
                                  np.zeros(self.mpc.rnn.obst.obstacle_num *self.horizon)])  
            ubx = np.concatenate([np.array(x).flatten(),self.X_upper_bound, 
                                  U_upper_bound,  
                                  np.inf*np.ones(self.mpc.rnn.obst.obstacle_num *self.horizon)])
            ...
            solution = self.solver_inst_random(
                p = cs.vertcat( 
