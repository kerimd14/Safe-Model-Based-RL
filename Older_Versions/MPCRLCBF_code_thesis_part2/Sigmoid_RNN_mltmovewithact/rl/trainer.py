...
class Trainer:
    ...
    Notes on viz:
      - `viz` is now a *module* (viz/viz.py), not a class instance.
      - We keep calling `viz.training_snapshot(...)`, `viz.plot_B_update(...)`, etc.
        This works for both a module and a simple namespace object.
    """
    ...
    def rl_trainingloop(
        ...
        # NOTE: keeping your original indexing scheme (starts at i=1)
        for i in range(1, total_steps):
            ...
            if k == episode_duration - 1:
                current_episode = i // episode_duration  # matches your existing convention
                ...
                if (current_episode % 50) == 0 and self.viz is not None:
                    ...
                    # IMPORTANT: CONSTRAINTS_X is now passed as-is (scalar or vector),
                    # because viz.training_snapshot supports both.
                ...
                # --- periodic evaluation ---
                if (current_episode % 5) == 0 and self.evaluator is not None:
