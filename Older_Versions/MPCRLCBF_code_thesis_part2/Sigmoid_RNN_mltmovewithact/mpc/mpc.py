...
class MPC:
    ...
    def __init__(
        ...
        flat_input = self.flat_input_fn(self.X_sym, self.xpred_hor, self.ypred_hor, self.U_sym)
        ...
        # theta vector for qp updates (you currently update ONLY rnn params)
    ...
    def make_flat_input_fn(self):
        ...
        for t in range(self.horizon):
            ...
            inter.extend(u_list)              # na scalars
    ...
    def generate_symbolic_mpcq_lagrange(self):
        ...
        # bounds (same style as your trainer uses)
        X_lower_bound = -np.tile(CONSTRAINTS_X, self.horizon)
        X_upper_bound = np.tile(CONSTRAINTS_X, self.horizon)

        # IMPORTANT:
        # you are fixing U[:,0] via lbx/ubx, and leaving the remaining horizon-1 free here
    ...
    def make_phi_fn(self):
        """
        Return a CasADi Function φ(X, U, xpred_hor, ypred_hor, h0, params)
        which returns a list of m×1 vectors, one per k.
